    ...
    with dpg.window(label='Transport Task Create',tag='createTaskWindow',width=400, height=200):
        with dpg.group(horizontal=True):
            with dpg.group(horizontal=False):
                with dpg.group(horizontal=True):
                    dpg.add_text('n:')
                    dpg.add_input_int(default_value=1, min_value=1, max_value=100,min_clamped=True,max_clamped=True, tag='input_n',width=100)
                with dpg.group(horizontal=True):
                    dpg.add_text('m:')
                    dpg.add_input_int(default_value=1, min_value=1, max_value=100,min_clamped=True,max_clamped=True, tag='input_m',width=100)
