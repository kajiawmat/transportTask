...
def fillTaskWindow(sender, val, user_data):
    if not dpg.does_item_exist('task_fill'):
        with dpg.window(label='Fill task', tag='fillTaskWindow', on_close=lambda: dpg.delete_item('fillTaskWindow')):
            ...
            with dpg.table(tag='transport_task', header_row=True, user_data=new_data, width=m * 150 + 150,
                ...
                for i in range(n):
                    with dpg.table_row():
                        dpg.add_text(f'A{i}')
                        for j in range(m):
                            dpg.add_input_int(default_value=matr_C[i][j], max_value=10000, min_value=0,
                                              min_clamped=True, max_clamped=True, user_data=(i, j),
                                              callback=cell_c_callback)
                        dpg.add_input_int(default_value=vect_A[i], max_value=10000, min_value=0, min_clamped=True,
                                          max_clamped=True, user_data=i, callback=cell_a_callback)
                with dpg.table_row():
                    dpg.add_text('Receivers')
                    for j in range(m):
                        dpg.add_input_int(default_value=vect_B[j], max_value=10000, min_value=0, min_clamped=True,
                                          max_clamped=True, user_data=j, callback=cell_b_callback)
