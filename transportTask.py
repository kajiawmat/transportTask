...
def taskFunc(matr_C, matr_X):
    res='F = '+' + '.join(
        sum_lists([[f'{matr_C[i][j]}*{val_X}' for j,val_X in enumerate(vect_X) if val_X!=-1] for i,vect_X in enumerate(matr_X)])
    )
    res+=' = '+str(sum(
        sum_lists([[matr_C[i][j]*val_X for j, val_X in enumerate(vect_X) if val_X != -1] for i, vect_X in enumerate(matr_X)])
    ))
    return res
